...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'Polaris', 'Polaris', and 'Polaris'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.Polaris/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.Polaris/g),
   ...
   * Rename files.
   *
   * Rename files that had Polaris in their name, such as
   * css/Polaris.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*Polaris*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('Polaris', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-Polaris-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Polaris's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace Polaris name mentions in files.
      generator.replaceFileContents();
      // Rename files that had Polaris in their name.
